...
def load_data(engine: create_engine, query: str) -> pd.DataFrame:
    """
...
def handle_price(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for replace_col in ['akciós ár', 'extrákkal növelt ár']:
        ...
        df_processed.loc[price_is_nan, 'vételár'] = df_processed.loc[price_is_nan, replace_col].values
    ...
    msk_eur_price = ((df_processed['vételár'] == df_processed['vételár eur']) &
                     (df_processed['vételár'] < 200000)).values
    euro_exchange_rate = 375
    count_of_invalid_price = np.sum(msk_eur_price)
    if np.sum(msk_eur_price) > 0:
        print(f'WARNING: {count_of_invalid_price} cars have inconsistent price value, EUR value is used!')
        df_processed.loc[msk_eur_price, 'vételár'] *= euro_exchange_rate

    return df_processed


def drop_high_nan_columns(df: pd.DataFrame, threshold: int = 1000) -> pd.DataFrame:
    """
    ...
    :param threshold: The minimum number of non-NaN values required for a column to be retained.
                      Defaults to 1000.
    ...
    high_nan_cols = nan_count[nan_count > dataset_size - threshold].keys()
...
def str_to_numeric(df: pd.DataFrame, columns: dict) -> pd.DataFrame:
    ...
    for column, dtype in columns.items():
        ...
        df_transformed[column] = df_transformed[column].str.replace(r'\D', '', regex=True).astype(dtype)
...
def remove_non_alpha(df: pd.DataFrame, column: str) -> pd.DataFrame:
    ...
    df_transformed = df.copy()
    df_transformed[column] = df_transformed[column].str.replace(r'[^a-zA-Z]', '', regex=True)
    return df_transformed
...
def convert_power_to_numeric(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_transformed = df.copy()
    df_transformed['teljesítmény'] = df_transformed['teljesítmény'].str.replace(r'kW.*', '', regex=True).str.replace(
...
def convert_creation_date_to_age(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_transformed = df.copy()
    df_transformed['évjárat'] = pd.to_datetime(df_transformed['évjárat'].str.replace(r'\(.+\)', '', regex=True))
    df_transformed['age (year)'] = (
                (pd.to_datetime('2023-06') - df_transformed['évjárat']).astype('timedelta64[D]') / 365).astype(int)
    df_transformed.drop(columns=['age_days'], errors='ignore', inplace=True)  # remove the age_days column if it exists
...
def convert_technical_license_validity_date(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_transformed['műszaki vizsga érvényes'] = (
                pd.to_datetime(df_transformed['műszaki vizsga érvényes']) - pd.to_datetime('2023-06')).astype(
        'timedelta64[D]')
...
def extract_tire_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    column_mappings = {
        'nyári gumi méret': ['summer tire width', 'summer tires aspect ratio', 'summer tires construction type',
                             'summer tires rim diameter'],
        'téli gumi méret': ['winter tire width', 'winter tires aspect ratio', 'winter tires construction type',
                            'winter tires rim diameter'],
        'hátsó nyári gumi méret': ['back summer tire width', 'back summer tires aspect ratio',
                                   'back summer tires construction type', 'back summer tires rim diameter'],
        'hátsó téli gumi méret': ['back winter tire width', 'back winter tires aspect ratio',
                                  'back winter tires construction type', 'back winter tires rim diameter']
    }

    for original_column, new_columns in column_mappings.items():
        df_transformed[new_columns] = df_transformed[original_column].str.extract(pattern)

    numeric_values_from_extractions = [
        'summer tire width', 'summer tires aspect ratio', 'summer tires rim diameter',
        'winter tire width', 'winter tires aspect ratio', 'winter tires rim diameter',
        'back summer tire width', 'back summer tires aspect ratio', 'back summer tires rim diameter',
        'back winter tire width', 'back winter tires aspect ratio', 'back winter tires rim diameter'
    ]
    df_transformed[numeric_values_from_extractions] = df_transformed[numeric_values_from_extractions].astype(float)
...
def filter_invalid_power_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    p_kw = df['teljesítmény'].str.replace(r'kW.*', '', regex=True).str.replace(r'\D', '', regex=True).astype(float)
    p_le = df['teljesítmény'].str.replace(r'.*?(\d+).LE', r'\1', regex=True).astype(float)
...
def filter_usable_features(feature_frequencies: pd.Series) -> List[str]:
    ...
    """
    speaker_counts = [4, 6, 8, 10, 12]
    unwanted_features = list(pd.Series(speaker_counts).astype(str) + ' hangszóró') + ['', 'kültér', 'műszaki', 'beltér',
...
def add_special_features(df: pd.DataFrame, usable_features: List[str]) -> pd.DataFrame:
    ...
    df_special = pd.DataFrame([determine_speaker_count(x) for x in without_description], columns=['speaker count'])
...
def load_hungarian_settlements(filepath: str) -> pd.DataFrame:
    ...
    df_hun['cleaned_settlement'] = df_hun['settlement'].str.replace(
        r'Budapest.+', 'Budapest', regex=True
    ).str.lower().str.strip()
    return df_hun
...
def extract_address_information(df: pd.DataFrame, settlements_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    address = pd.Series(np.where(mask, df.content_info_1, df.content_info_0)).str.lower()
    address = address.str.replace('térkép megtekintése', '', regex=False)

    zip_code = address.str.replace('[^\d ]', '', regex=True).str.extract("( \d\d\d\d )")[0].astype(float)
    ...
    df['city'] = address_without_numbers.str.extract(f"( {' | '.join(settlements_df['cleaned_settlement'].unique())} )")
...
def process_financing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df['financing'] = df['financing'].str.replace(r'\D', '', regex=True).replace({None: '100'}).astype(int)
    return df
...
def apply_tfidf_vectorization(df: pd.DataFrame, stop_words: List[str], max_features: int = 50,
                              vectorizer_file: str = 'vectorizer.joblib', update: bool = False) -> pd.DataFrame:
    """
    ...
    tfidf_df.rename(columns={'tfidf_sports': 'tfidf_sport'}, errors='ignore', inplace=True)
...
def main(
    ...
    df = apply_tfidf_vectorization(df, stop_words, max_features=20, vectorizer_file=vectorizer_file)
